...
import { MedicalRecordInsert, MAX_DATOS_DINAMICOS } from '@features/medical-history/models/medical-record.model';
...
export class FinishAppointmentDialogComponent implements OnInit, OnDestroy {
  ...
  /** Indica si el formulario es valido para habilitar el boton de envio. */
  ...
  async confirmar(): Promise<void> {
    ...
    const payload: MedicalRecordInsert = {
      ...
      datos_dinamicos: (valores.datos_dinamicos ?? []).map((d: { clave: string; valor: string }) => ({
        clave: d.clave.trim(),
        valor: d.valor.trim(),
      })),
    };
